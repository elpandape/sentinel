...
.PHONY: build install update test test-quiet bench coverage types stan lint lint-fix rector rector-fix mutation validate ci shell test-dbs
...
test-dbs: ## Suite against MySQL & Postgres (make test-dbs ARGS=tests/Capture recreates the schema, waits for healthchecks)
